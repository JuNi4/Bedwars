...
$execute if score bed_air data matches 1.. if entity @a[team=!$(team), distance=..6.5, limit=1,sort=nearest, scores={statistic_beds_destroyed_unchecked=1..}] as @a[team=!$(team)] at @s run playsound minecraft:entity.ender_dragon.growl ambient @s
$execute if score bed_air data matches 1.. if entity @a[team=!$(team), distance=..6.5, limit=1,sort=nearest, scores={statistic_beds_destroyed_unchecked=1..}] run function bedwars:game/bed_destroyed {team:$(team), team_name:$(team_name), team_col:$(team_col)}
