# spawn everyone who left
function bedwars:setup/on_join
...
# triggers
execute as @a[scores={start=1..}] store result score start_in data run scoreboard players get @s start
execute if entity @a[scores={reset=1..}] run function bedwars:reset


# run the main tick function if the game is running
execute if score is_running data matches 1 run return run function bedwars:game/tick
