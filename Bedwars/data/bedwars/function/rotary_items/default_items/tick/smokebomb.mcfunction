...
execute at @e[tag=smoke_bomb_init] run summon arrow ~ ~ ~ {Tags:["smoke_bomb", "smoke_bomb_arrow_init"],life:1000,item:{id:"minecraft:tipped_arrow",count:1,components:{"minecraft:potion_contents":{custom_effects:[{id:"minecraft:blindness",amplifier:255,duration:1000,show_particles:1b,show_icon:0b},{id:"minecraft:slowness",amplifier:0,duration:500,show_particles:0b,show_icon:0b}]}}}}
execute as @e[tag=smoke_bomb_arrow_init] at @s run data modify entity @s Owner set from entity @p UUID
tag @e[tag=smoke_bomb_arrow_init] remove smoke_bomb_arrow_init
