...
execute as @e[tag=emetic_granade_init] at @s run summon arrow ~ ~ ~ {Tags:["emetic_granade", "emetic_granade_arrow_init"],life:800,item:{id:"minecraft:tipped_arrow",count:1,components:{"minecraft:potion_contents":{custom_effects:[{id:"minecraft:nausea",amplifier:0,duration:2000}]}}}}
execute as @e[tag=emetic_granade_arrow_init] at @s run data modify entity @s Owner set from entity @p UUID
tag @e[tag=emetic_granade_arrow_init] remove emetic_granade_arrow_init
